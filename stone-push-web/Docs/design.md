...
| `applyAggressiveMove(state, move)` | アグレッシブ移動を適用（押し出し処理）→ 勝利判定 → ターン交代 or `GAME_OVER`。ターン交代後に次の手番の合法なリードが1つも無ければ、その場で次の手番側の反則負けとして`GAME_OVER`にする（完全デッドロック対策） |
...
石数による勝敗が付かなかった場合でも、ターン交代後の次の手番側に合法なリードが1つも無ければ（完全デッドロック）、その時点で`legalPassiveMoves(nextState)`が空であることを検知し、次の手番側の反則負け（直前に動いた`player`の勝利）として`GAME_OVER`にする。これにより、手の無いプレイヤーへターンが渡ってCPU側が例外を投げる・人間側の画面が何も選択できず固まったように見える、という状態を回避している。
...
`legalPassiveMoves` が既にデッドエンドを除外済みであることに加え、`applyAggressiveMove`側で「次の手番の合法なパッシブ移動が1つも無い（完全デッドロック）」場合は手番交代前にその場で`GAME_OVER`にしてしまうため（4章参照）、`chooseCpuTurn`が呼ばれる時点で`legalPassiveMoves`は必ず1件以上の候補を返すことが保証されている。
...
- `main.ts`の`render()`冒頭で`document.documentElement.lang = lang`を毎回設定し、`<html lang>`属性を表示言語と同期させている（アクセシビリティ・SEO対策。以前は`index.html`の初期値`en`のまま固定されていた）
