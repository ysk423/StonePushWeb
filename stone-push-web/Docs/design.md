...
└── stone-push-web/          # アプリ本体（Vite プロジェクトルート）
    ...
    ├── src/
    │   ├── main.ts            # エントリーポイント／アプリ状態管理／CPU手番トリガー／言語・難易度state
...
- `lang` / `cpuDifficulty` は `AppState` ではなく `main.ts` のモジュールスコープに独立して保持している（画面遷移・ゲーム状態のリセットとは無関係にアプリ全体で共有される設定のため）。永続化（`localStorage`）は未実装で、リロードすると既定値（英語／よわい）に戻る
...
- **戦績・設定の永続化**：`localStorage` への読み書きを行う `data/` 相当のモジュールを追加し、`main.ts` から呼び出す想定（現状は未着手。`lang` / `cpuDifficulty` もリロードで既定値に戻る）
