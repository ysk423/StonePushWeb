...
Firebase（Firestore）関連の設定ファイルはアプリ本体（`stone-push-web/`）直下に置き、`npx firebase-tools deploy --only firestore:rules` / `firestore:indexes` のようにアプリのディレクトリで直接デプロイできるようにしている。プロジェクト作成・Firestore有効化・セキュリティルール/インデックスの内容と運用手順は `LEADERBOARD.md` に詳細をまとめてある。
...
Firebase（Firestore）に対局結果を保存し、誰でも閲覧できるランキングを提供する機能。Firebase側の設定内容・運用手順は `LEADERBOARD.md` に詳細をまとめてあり、ここではアプリ側の実装方針のみ記す。
...
- `src/records.ts`：`submitRecord(record)` / `fetchRanking(category)` / `categoryFor(mode, difficulty)` の3関数のみを公開する。Firestoreのクエリ組み立て（`where` + `orderBy` × 2 + `limit`）やコレクション名（`records`）といった詳細はこのモジュール内に閉じ込め、呼び出し側（`main.ts`）はFirestoreのAPIを一切知らなくてよい
...
- **記録対象の判定**（`shouldOfferRecord`、`render.ts`内）：`game.mode === 'VS_HUMAN' || game.winner === game.humanPlayer`。vs人間はどちらが勝っても人間の記録、vs CPUは人間側が勝った時のみ記録対象（CPU自身の勝利は記録しない）
...
- **戦績ランキングの不正対策強化**：Cloud Functions経由での書き込み・Firebase Anonymous Authの導入など。詳細は`LEADERBOARD.md` 7章
