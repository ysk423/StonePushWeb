...
└── stone-push-web/          # アプリ本体（Vite プロジェクトルート）
    ...
    ├── src/
    │   ├── main.ts            # エントリーポイント／アプリ状態管理／CPU手番トリガー／言語state
...
GitHub Actionsのワークフロー（`deploy.yml`）は `working-directory: stone-push-web` を指定して `npm ci` / `npm run build` を実行し、`stone-push-web/dist` を `actions/upload-pages-artifact` で公開する構成（詳細は `GITHUB_PAGES_DEPLOY.md` 8章）。
...
Android版の `GameState` と異なり、`selectedPassiveFrom` / `passiveMove` / `selectedAggressiveFrom` を state 自体に持たせることで、「選択中の石（パッシブ／アグレッシブ双方）」「確定済みパッシブ移動（方向・歩数の引き継ぎ＆キャンセル時の巻き戻し用）」をViewModel層を介さず素直に表現している（Web版にはViewModel層が無く、`GameState` がその役割も兼ねる）。
...
`render()` はステート変更のたびに1回しか呼ばれないため、CPU手番の間だけ重複してタイマーが積み上がることはない（CPU手番中は人間側の操作が無効化されており、次にrenderが呼ばれるのはCPUの手が適用された後＝再びcurrentPlayerが人間になった後）……という前提だったが、リード→フォローを2段階アニメーションにした際、中間`setState`（リード確定時点）が`render()`を再度呼び出し、`scheduleCpuTurnIfNeeded`が二重に発火しうることが判明したため、**`isAnimating`が`true`の間は`scheduleCpuTurnIfNeeded`の本体処理自体をスキップする**ガードを追加している（下記参照）。

CPUの手はパッシブ移動（リード）→アグレッシブ移動（フォロー）の順に、**同じ速度（`PUSH_ANIMATION_MS`=260ms）で連続してスライドアニメーション**させてから確定する。以前はリードのみ無演出即時反映だったため「リードとフォローでアニメーション速度が違う」というフィードバックがあり、リード側にも同じ速度のスライドを追加して揃えた。
...
- `renderStart` / `renderRules` / `renderGame` の3つのトップレベル関数のみ
...
4枚のボードの中央交点（`--border-line` の交差部分）には `.border-label` で「ボーダー」というテキストラベルを常時表示している。ルール説明画面の図解でも同じ用語・同じ色で説明しており、対局画面と用語を一致させている。
...
- `lang`は`AppState`ではなく`main.ts`のモジュールスコープに独立して保持している（画面遷移・ゲーム状態のリセットとは無関係にアプリ全体で共有される設定のため）。永続化（`localStorage`）は未実装で、リロードすると既定の英語に戻る
- 既定言語は英語（`let lang: Lang = 'en'`）
- `render.ts`側の`renderStart` / `renderGame` / `renderRules`はすべて`lang: Lang`を引数に取り、内部で`getDict(lang)`した`Dict`をテンプレート文字列の差し込みに使うだけで、ロジック分岐は一切持たない（文言の言語分岐を`render.ts`本体に持ち込まない設計）

---

## 10. 未実装だが設計上考慮している拡張ポイント
